...
[`src/daemonRPC.php`](https://github.com/uplexa/uplexa-php/tree/master/src/daemonRPC.php)

A class for making calls to a uPlexa daemon's RPC API using PHP

Parameters:

 - `$host <String>` uPlexa daemon port *(optional)*
 - `$port <nNmber>` uPlexa daemon protocol (*eg.* 'http') *(optional)*
 - `$protocol <String>` uPlexa daemon IP hostname *(optional)*
 - `$user <String>` uPlexa daemon RPC username *(optional)*
 - `$password <String>` uPlexa daemon RPC passphrase *(optional)*
